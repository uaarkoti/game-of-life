...
node('docker') {
    docker.withServer('tcp://docker.jenkins.local:2376') {
        docker.image('uday/maven:3.3.3-jdk-8').inside('-v /data:/data') {
            sh 'rm -rf *'
            checkout([$class: 'GitSCM', branches: [[name: '*/master']], clean: true, doGenerateSubmoduleConfigurations: false, extensions: [], submoduleCfg: [], userRemoteConfigs: [[url: 'https://github.com/uaarkoti/game-of-life.git']]])
            sh 'git checkout master'
            sh 'git config user.email "uaarkoti@gmail.com"'
            sh 'git config user.name "uaarkoti"'
            sh 'git remote set-url origin git@github.com:uaarkoti/game-of-life.git'
...
node('docker') {
    ...
    docker.withServer('tcp://docker.jenkins.local:2376', 'slave-docker-us-east-1-tls'){
        ...
        dir('target') {
            mobileDepositApiImage = docker.build "uaarkoti/game-of-life:${buildVersion}"
        }

        stage 'Deploy to Prod'
        try{
          sh "docker stop game-of-life"
          sh "docker rm game-of-life"
        } catch (Exception _) {
           echo "no container to stop"
        }
        mobileDepositApiImage.run("--name game-of-life -p 9090:9090")
        sh 'curl http://docker.jenkins.local:8080/docker-traceability/submitContainerStatus --data-urlencode inspectData="$(docker inspect game-of-life)" --data-urlencode hostName=prod-server-' + rand.nextInt(max+1)
